                ...
                <div class="row mb-2">
                    <div class="col-md-10">
                            ...
                            <div class="col-md-4">
                                <select class="form-control" name="gender" required>
                                    ...
                                    @foreach($gender as $data_gender)
                                        <option value="{{ $data_gender->id  }}" @if($data_gender->id == $user->gender)
                                            {{ "selected" }}
                                                @endif>{{ $data_gender->title }}</option>
                                    @endforeach
                                </select>
                                @error('gender')
                                <small class="text-danger">{{ $message }}</small>
                        ...
                        <div class="row mb-1">
                            ...
                            <div class="col-md-4">
                                <input type="text" name="tempat_kerja" class="form-control" required
                                       value="{{ old('tempat_kerja', $user->tempat_kerja) }}">
                                @error('tempat_kerja')
                                <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4">
                                <select class="form-control" name="status_pekerjaan" required>
                                    ...
                                    @foreach($jenis_pekerjaan as $data)
                                        <option @if($user->status_pekerjaan == $data->id)
                                                    {{ "selected" }}
                                                @elseif(old('status_pekerjaan') == $data->id )
                                                    {{ "selected" }}
                                                @endif value="{{ $data->id }} ">{{ $data->title }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-1">
                            ...
                            <div class="col-md-4">
                                <select class="form-control" name="status_menikah" required>
                                    ...
                                    @foreach($stutus_menikah as $sm)
                                        <option @if($user->status_menikah == $sm->id)
                                                    {{ "selected" }}
                                                @elseif(old('status_menikah') == $sm->id )
                                                    {{ "selected" }}
                                                @endif value="{{ $sm->id }}">{{ $sm->title }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                <select class="form-control" name="agama" required>
                                    <option value="">------</option>
                                    @foreach($agama as $ag)
                                        <option @if($user->agama == $ag->id)
                                                    {{ "selected" }}
                                                @elseif(old('agama') == $ag->id )
                                                    {{ "selected" }}
                                                @endif value="{{ $ag->id }} ">{{ $ag->title }}</option>
                                    @endforeach
                                </select>
                                @error('status_menikah')
                                <small class="text-danger">{{ $message }}</small>
